 ...
 * @covers \ClickNow\Checker\Console\Command\Git\PrePushCommand
 * @covers \ClickNow\Checker\Console\Command\AbstractRunnerCommand
 * @runTestsInSeparateProcesses
 */
class PrePushCommandTest extends \PHPUnit_Framework_TestCase
{
    /**
    ...
    protected function setUp()
    {
        ...
        $application->add(new PrePushCommand($this->runner, $this->git));

        $this->runnerHelper = m::spy(RunnerHelper::class);

        $command = $application->find('git:pre-push');
    ...
    {
        $this->git->shouldReceive('getCommittedFiles')->with(null)->once()->andReturn(new FilesCollection());
        $this->runnerHelper->shouldReceive('run')->with(m::type(PrePushContext::class))->once()->andReturn(0);
    ...
    {
        $this->git->shouldReceive('getCommittedFiles')->with(null)->once()->andReturn(new FilesCollection());
        $this->runnerHelper->shouldReceive('run')->with(m::type(PrePushContext::class))->once()->andReturn(1);
